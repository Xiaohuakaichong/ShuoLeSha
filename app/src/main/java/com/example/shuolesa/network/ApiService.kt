...
必须输出严格合法的纯 JSON 格式：
{
  ...
  "reminders": ["时间敏感的提醒，如明天下午三点回电客户"],
  "notepad": "非待办的杂记备忘（购物灵感、随口提到的书名/链接、需要查一下的事等；若无则空字符串）",
  "memory": ["值得长期记住的事实/偏好/承诺，如'小王下周搬家'、'我对生蚝过敏'"],
  "daily_quote": "基于今天一切经历提炼的今日心境金句/人生感悟"
}
注意：
1. 仔细提取散落在各段录音中的所有行动待办，合并归纳到 unified_action_items；
2. 敏锐捕捉人际闲聊中的温情与幽默，让闲聊像电影切片一样被永久记录；
3. reminders 只放有时间或跟进压力的提醒；notepad 放零散文案；memory 放跨日仍有价值的长期记忆；三者均可为空数组或空字符串；
4. 必须输出且仅输出合法标准 JSON，不要带有 markdown 标记或多余文字。"""
